...
### Subagent Tool Restrictions Live in `tools:`, Not `allowed-tools:`
**Where it came up:** Grading the four subagents in `.claude/agents/` against Domain 1 / Task Statement 1.3 of the Claude Certified Architect exam ("tool restrictions for each subagent type"). Two of them — `persistence-safety-reviewer` and `pip-chat-safety-evaluator` — declared `allowed-tools: Read, Grep, Glob, Bash` and stated "read-only, do not modify files" in their prompts, yet the Claude Code agent listing showed all four loading as **All tools**.
**What it is:** `allowed-tools` is the **Skill** (`SKILL.md`) frontmatter key. The subagent schema uses **`tools`**, whose documented rule is "inherits every tool available to subagents if omitted." An unrecognized YAML key is not an error — it is silently ignored — so the restriction evaporated and both agents loaded with the full toolset, `Write` and `Edit` included. The companion field is `disallowedTools` (a denylist, applied first; `tools` is then resolved against what remains, so a tool named in both is removed).
**In our code:** Fixed in `7576d75` — one key rename per file, verified against the official subagent field table rather than from memory. Both agents keep `Bash`, so `persistence-safety-reviewer` can still run `git diff` and `pip-chat-safety-evaluator` can still run `eval/pip_eval.py`. `code-reviewer` and `adaptive-layout-engineer` were deliberately left inheriting everything — though `code-reviewer`'s prompt says "do not apply edits" while it still holds `Edit`, which is the same prompt-only enforcement one level down.
**Why it matters:** The two agents guarding the highest cost-of-failure surfaces in this project — kids' saved SwiftData progress, and the prompt a six-year-old talks to — were enforcing "read-only" with a *sentence* instead of a *schema*. That is the same distinction `bash-guard.py` exists to make: prose has a non-zero failure rate, structure has none. The more general lesson is the failure mode: a mistyped config key produces no error, no warning, and a silently weaker system. When a config field is load-bearing for safety, verify it actually took effect — writing it is not the same as it working.

### Subagents Start Blind — Context Is Handed Over or Re-Derived, Never Inherited
**Where it came up:** The same exam-prep pass, Task Statements 1.2/1.3 — "subagents do not automatically inherit parent context or share memory between invocations."
**What it is:** Each subagent runs in its own context window. It sees its own system prompt and whatever the coordinator hands it — not the conversation that spawned it, not what a sibling subagent just found, not anything from a previous invocation of itself. The architecture is hub-and-spoke: every result routes back through the coordinator, which is what keeps information flow and error handling observable. Two practical consequences: the coordinator must put complete prior findings *inside* the next subagent's prompt, and parallel subagents are spawned by emitting multiple Task calls in ONE response — structurally the same "all in one message" rule as parallel tool use.
**In our code:** Our reviewers are written correctly for this. `persistence-safety-reviewer` opens with "`git diff` (and `--cached`) to find changed persisted types — **or review the files named by the caller**", naming both survival strategies: re-derive state from durable sources, or accept an explicit handoff. `code-reviewer` does the same ("use git diff or file timestamps to find what changed"). Neither assumes it was told anything.
**Why it matters:** The failure mode is a subagent prompt that reads perfectly sensibly and starves at runtime — "review the changes we discussed" hands over nothing, because from the subagent's side there is no "we." Design every subagent to either re-derive its inputs from durable state (git, the filesystem) or to state exactly what the caller must pass in. The related trade-off is scope: `code-reviewer`'s "focus ONLY on recent changes" buys attention and buys blind spots — a regression caused by an *unchanged* file is outside its window by construction, which is the "overly narrow decomposition" risk the exam names.

### `stop_reason` Is the Loop's Control Flow — the Text Is Not
**Where it came up:** Studying Domain 1 / Task Statement 1.1 of the Claude Certified Architect exam ("design and implement agentic loops"), then grading `askCloud` against it instead of a textbook example.
**What it is:** An agentic loop is not "call the model, get an answer." It is: send the request → read `stop_reason` → if `"tool_use"`, run the tool and go around again; if `"end_turn"`, stop. `stop_reason` is a structured field on the response, which is the whole point — it's a machine-readable signal, not something you infer. The trap is that a `tool_use` response can *also* contain text ("Let me peek at your garden…"), so "there is text, therefore it's finished" is wrong. The API is stateless, so the loop's only memory is the `messages` array you rebuild and resend each iteration.
**In our code:** `PipAIService.swift` reads `stop_reason` out of the `message_delta` SSE event at :913, then branches on it: :933 short-circuits a `"refusal"` into the canned `refusalReply`, :940 continues the loop on `"tool_use"`, and anything else falls through to the final-text path. The later `guard !trimmed.isEmpty` is a *validity* check on the answer, not the termination condition — worth keeping straight, because conflating them is exactly the anti-pattern.
**Why it matters:** The three anti-patterns the exam names are all the same mistake wearing different hats — using something other than `stop_reason` to decide you're done: parsing the reply's wording, counting iterations, or treating assistant text as a completion flag. Beyond the exam: a loop keyed on anything but the structured field breaks silently the moment the model phrases itself differently, and silent breakage in a kids' chat means Pip answers half a question and stops.

### A "Cap" Is an Emergency Brake, Not a Steering Wheel
**Where it came up:** The exam guide lists "setting arbitrary iteration caps as the primary stopping mechanism" as an anti-pattern, and our loop has `maxHops = 2` — so: is our loop wrong?
**What it is:** A cap is a hard ceiling that stops something running away from you — same idea as `paidDailyLimit = 20`, the daily question cap already in this file. An *iteration* cap is that applied to a loop: never go around more than N times, so a model that kept requesting tools forever can't spin the loop, burn money, and hang the app. The exam's objection is narrower than it first reads: the operative words are **"as the primary stopping mechanism."** A cap is wrong when it *replaces* `stop_reason` logic, fine when it sits underneath it as a backstop.
**In our code:** `askCloud` caps at `maxHops = 2` (:767), but the cap never truncates a reply. `let allowTools = (hop < maxHops)` (:777) makes the final hop send `"tool_choice": ["type": "none"]` (:794), which *forces* the model to answer in text. So hitting the cap changes **how** the turn ends, never **whether** the kid gets a reply — `stop_reason` is still the steering wheel.
**Why it matters:** Transferable test for reviewing any capped loop, on the exam or in a PR: don't judge the cap's existence, judge what happens when it's reached. Silently returning a partial or empty result is the bug. Converting "budget exhausted" into a guaranteed clean final turn is good engineering. The same shape shows up in retry limits, pagination guards, and recursion depth — the ceiling is fine; using the ceiling as the logic is not.

### Parallel Tool Use: All Results Go Back in ONE User Message
**Where it came up:** Grading the loop turned up a real functional limit — `maxHops = 2` plus `disable_parallel_tool_use: true` meant Pip could make at most **one** tool call per question, ever. A kid asking "what can I cook with what I'm growing?" would get one lookup and have to invent the other half, which is precisely what the `<truthfulness>` system-prompt rule forbids.
**What it is:** By default Claude may request several tools in a single assistant turn. The wire protocol for answering them has a rule that is easy to get wrong: you echo the assistant turn back containing *every* `tool_use` block, then reply with a **single** user message containing *every* matching `tool_result` block (paired by `tool_use_id`). Splitting those results across separate user messages technically parses, but it trains the model to stop requesting tools in parallel — the behavior degrades quietly rather than erroring.
**In our code:** Removing `disable_parallel_tool_use` from the `tool_choice` dict (:794) was the entire functional change — the loop already built the correct shape: one `tool_use` block per tool on the assistant turn, then all `tool_result` blocks collected into `userContent` and appended as one message at :980. Because both lookups now fit in the single tool-calling turn, `maxHops` did **not** need raising. The UI followed: `toolInFlightMessage(for:)` (:370) now joins per-tool subjects from `toolSubject(name:)` (:358) into one sentence — "Pip is looking at your garden and your recipes..." — set once before any tool runs (:966), because the tools execute back-to-back and a per-tool line was being overwritten faster than a child could read it. Verified on device: a question spanning both tools produced two `[PipAI] TOOL …` lines in a single turn.
**Why it matters:** Two lessons. First, a capability flag and a loop bound can combine into a limit neither one states on its own — `maxHops = 2` looked generous until you noticed the last hop can't call tools and the first could only call one. Second, when a protocol has a "put them all in one message" rule, the failure mode is usually behavioral rather than a thrown error, so you won't learn about it from a crash — you learn it from reading the contract. *(Supersedes the "Anthropic Tool Use + Prompt Caching (Phase 1)" entry below on two points: `tool_choice` no longer disables parallel tool use, and hop 2 keeps the `tools` key rather than omitting it — dropping `tools` would invalidate the cached prefix.)*
